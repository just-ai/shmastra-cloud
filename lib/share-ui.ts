// Inline share-overlay UI. Served as a `<script>…</script>` injected by the
// owner route. We can't ship it as `<script src="…">` because the page has a
// `<base href="https://<sandbox>/…">`, which would route the asset away from
// cloud.com. Keeping the source here avoids that.

const SHARE_UI_SCRIPT = `
(function() {
  if (window.__shmastraShareUI) return;
  window.__shmastraShareUI = true;

  var APP_NAME = window.__SHMASTRA_OWNER_APP_NAME || "";
  var CLOUD_URL = window.__SHMASTRA_CLOUD_URL || window.location.origin;
...
export function getShareUiScript(appName: string, cloudUrl: string): string {
  return (
    `<script>` +
    `window.__SHMASTRA_OWNER_APP_NAME=${JSON.stringify(appName)};` +
    `window.__SHMASTRA_CLOUD_URL=${JSON.stringify(cloudUrl)};` +
    `</script>` +
    `<script>${SHARE_UI_SCRIPT}</script>`
  );
}
