...
const template = Template({fileContextPath: sandboxDir})
    ...
    .aptInstall(["inotify-tools", "jq"], { noInstallRecommends: true })
    .setUser("user")
    .copy("ecosystem.config.cjs", `${homeDir}/ecosystem.config.cjs`)
    .copy("healer.mts", `${homeDir}/healer.mts`)
    .copy("project-watcher.sh", `${homeDir}/project-watcher.sh`)
    .copy("start.sh", startScriptPath)
    .runCmd(`chmod +x ${startScriptPath} ${homeDir}/project-watcher.sh`)
    .makeDir(appDir)
    .setWorkdir(appDir)
