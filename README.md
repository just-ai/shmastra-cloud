...
Next.js app: Mastra Studio behind WorkOS auth, E2B sandboxes, Supabase, and an LLM proxy.
...
- Add `https://<your-domain>/api/auth/callback` as a redirect URI in the WorkOS dashboard.
- Studio build vars (`MASTRA_*`): defaults live in committed `.env`; see [.env.example](.env.example) for the full template and optional overrides in Vercel.
